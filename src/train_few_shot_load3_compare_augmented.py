...
FIGURE_DIR = PROJECT_ROOT / "results" / "figures" / "few_shot_load3_augmented"

EPOCH_LOG_PATH = LOG_DIR / "few_shot_load3_compare_augmented_epoch_log.csv"
SUMMARY_PATH = LOG_DIR / "few_shot_load3_compare_augmented_summary.csv"

ACC_FIG_PATH = FIGURE_DIR / "few_shot_load3_augmented_accuracy_curve.png"
F1_FIG_PATH = FIGURE_DIR / "few_shot_load3_augmented_macro_f1_curve.png"
...
def evaluate_source_validation(model, val_loaders, criterion, device):
    """
    只用 source loads 的验证集选 best epoch。
    Load 3 不参与选择。
    """
    val_losses = []
    val_accs = []
...
def train_one_setting(model_name, train_ratio, device):
    ...
    Validation:
        Load 0+1+2 的 test set，不增强，用于选 epoch。

    Final Test:
        Load 3 test set，不增强，只最终测试一次。
    """
    set_seed(SEED)
    ...
    print("=" * 80)
    print(f"AUGMENTED few-shot Load3 experiment | Model: {model_name} | Ratio: {train_ratio}")
    print("=" * 80)
    print("Device:", device)
...
def main():
    ...
    print("=" * 80)
    print("AUGMENTED strict few-shot cross-load comparison")
    ...
    plot_metric(
        ...
        ylabel="Accuracy",
        title="Augmented STRICT Few-shot Cross-load Accuracy: Final Test Load 3",
    ...
    plot_metric(
        ...
        ylabel="Macro-F1",
        title="Augmented STRICT Few-shot Cross-load Macro-F1: Final Test Load 3",
        save_path=F1_FIG_PATH,
    )

    print("=" * 80)
    print("AUGMENTED strict few-shot Load3 comparison finished.")
