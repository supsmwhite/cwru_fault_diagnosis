...
EPOCH_LOG_PATH = LOG_DIR / "cross_load_cnn1d_epoch_log.csv"
SUMMARY_PATH = LOG_DIR / "cross_load_cnn1d_summary.csv"

ACC_MATRIX_FIG_PATH = FIGURE_DIR / "cnn1d_cross_load_accuracy_matrix.png"
F1_MATRIX_FIG_PATH = FIGURE_DIR / "cnn1d_cross_load_macro_f1_matrix.png"
...
def train_source_load(source_load, device):
    """
    在 source_load 上训练一个 CNN1D。
    每个 epoch 结束后，在 Load 0/1/2/3 的 test set 上全部评估。

    模型选择策略：
    - 用 source_load 自己的 test_acc 选择 best epoch；
    - 然后记录该 epoch 下对所有 target load 的表现；
    - 避免用目标负载测试集来反向选择模型。
    """
    set_seed(SEED)
    ...
    print("=" * 80)
    print(f"Cross-load CNN1D | Train Load {source_load} hp")
    print("=" * 80)
    print("Device:", device)
...
def main():
    ...
    print("=" * 80)
    print("Day 6: CNN1D cross-load generalization experiments")
    print("=" * 80)
    print("Device:", device)
    print("Epochs:", EPOCHS)
    ...
    plot_matrix(
        acc_matrix,
        "CNN1D Cross-Load Accuracy Matrix",
        ACC_MATRIX_FIG_PATH,
    )

    plot_matrix(
        f1_matrix,
        "CNN1D Cross-Load Macro-F1 Matrix",
        F1_MATRIX_FIG_PATH,
    )

    print("=" * 80)
    print("Cross-load CNN1D experiments finished.")
